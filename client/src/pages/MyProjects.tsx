...
const MyProjects = () => {
    ...
    const fetchProjects = async () =>{
        setProjects(dummyProjects) //import dummyProjects from the assests here 
        //simulate loading
        setTimeout(() => {
            setLoading(false)
        }, 1000);
            ...
            {loading ? (
                ...
                <div className='py-10 min-h-[80vh]'>
                    <div className='flex items-center justify-between mb-12'>
                        ...
                        sm:px-6 py-1 sm:py-2 rounded bg-linear-to-br from-indigo-500 to-indigo-600 hover:opacity-90 active:scale-95 transition-all'>
                    ...
                    <div className='flex flex-wrap gap-3.5'>
                        {projects.map((project)=>(
                            <div onClick={()=>navigate(`/projects/${project.id}`)} key={project.id} className='relative group w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden shadow-md group hover:shadow-indigo-700/30 hover:border-indigo-800/80 transition-all duration-300'>
                                {/*desktop type image */}
                                <div className='realtive w-full h-40 bg-gray-900 overflow-hidden border-b borger-gray-800'>
                                ...
                                <div className='p-4 text-white bg-linear-180 from-transparent group-hover:from-indigo-950 to-transparent transition-colors'>
                                    <div className='flex items-start justify-between'>
                                        <h2>{project.name}</h2>
                                        <button className='px-2.5 py-0.5 mt-1 ml-2 text-xs bg-gray-800 border border-gray-700 rounded-ful89+'>Website</button>
                                    </div>
